...
export default function VideoSourcePicker({ source, previewUrl, fileName, onSourceChange, onUpload, onClear }: {
  source: "library" | "upload"; previewUrl: string | null; fileName: string
  ...
  return <>
    ...
    {source === "library" ? (
      ...
      <label className={s.uploadBox}><input type="file" accept="image/*" onChange={(event) => onUpload(event.target.files?.[0])} /><Upload /><b>Sube tu imagen de referencia</b><small>JPG, PNG o WEBP · No se sube al servidor</small></label>
    )}
