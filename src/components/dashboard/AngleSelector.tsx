        ...
        {ANGLES.map((angle) => {
          ...
          return (
            <button
              ...
              disabled={loading}
              className={`group relative w-full min-h-32 text-left p-4 rounded-2xl border-2 transition-all duration-200 flex flex-col ${
                isSelected
                  ? "border-[#D97757] bg-[#D97757]/10 shadow-lg shadow-[#D97757]/10"
                  : "border-[#3A3833] hover:border-[#D97757]/50 hover:bg-[#1E1C1A]"
              } disabled:opacity-50 disabled:cursor-not-allowed`}
            >
              {isSelected && (
                <span className="absolute top-3 right-3 flex items-center justify-center w-5 h-5 rounded-full bg-[#D97757] text-white">
              ...
              <span
                className={`flex items-center justify-center w-10 h-10 rounded-xl text-xl flex-shrink-0 mb-3 transition-colors ${
                  isSelected ? "bg-[#D97757]/20" : "bg-[#1E1C1A] group-hover:bg-[#D97757]/10"
                }`}
              >
                {angle.icon}
              </span>
              <h4 className="text-sm font-bold text-[#E8E6E1] mb-1 pr-5">
                {angle.title}
              </h4>
